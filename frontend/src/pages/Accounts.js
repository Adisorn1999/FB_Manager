          ...
          <tbody>
            {paginated.map((acc) => (
              <tr key={acc.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-3 truncate max-w-[180px]">{acc.password}</td>
                ...
                <td className="px-4 py-3 text-center">
                  <button onClick={() => openEdit(acc)} className="text-blue-600 mr-2">Edit</button>
                  <button onClick={() => handleDelete(acc.id)} className="text-red-600 mr-2">Delete</button>
                  <button onClick={() => autoCopy(acc)} className="text-green-600">Copy</button>
                </td>
      ...
      {openModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center">
          ...
          <div className="bg-white p-6 rounded w-[400px]">
            ...
            {['username','password','secret_code','email','email_password','temp_mail','bm'].map(f => (
              <input
                key={f}
                value={form[f] || ''}
            ...
            {isDuplicate && <p className="text-red-600 font-bold">❌ Duplicate</p>}
