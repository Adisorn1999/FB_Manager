        ...
        {accounts
          ...
          .map((acc) => (
            <div key={acc.id} className="bg-white p-4 rounded shadow">
              ...
              <div className="text-sm text-gray-500 mt-1">BM: {acc.bm}</div>

              <div className="flex gap-3 mt-3">
                <button onClick={() => openDetail(acc)}>👁️</button>

                <button onClick={() => openAdd("page", acc)}>📄</button>

                <button onClick={() => openAdd("pixel", acc)}>⚡</button>

                <button onClick={() => openAdd("card", acc)}>💳</button>
              </div>
            </div>
          ))}
      </div>
      ...
      {addModal && (
        <Modal onClose={() => setAddModal(null)}>
          ...
          >
            ...
            {type === "page" &&
              pages.map((p) => {
                const used = accountPages.some((i) => i.page_id === p.page_id);

                return (
                  <option key={p.id} value={p.page_id} disabled={used}>
                    {p.page_id}
                    {used ? " (ใช้แล้ว)" : ""}
                  </option>
                );
            ...
            {type === "pixel" &&
              pixels.map((px) => {
                const used = accountPixels.some((i) => i.px_id === px.px_id);

                return (
                  <option key={px.id} value={px.px_id} disabled={used}>
                    {px.px_id}
                    {used ? " (ใช้แล้ว)" : ""}
                  </option>
                );
            ...
            {type === "card" &&
              cards.map((c) => {
                const used = accountCards.some((i) => i.number === c.number);
      ...
      {editModal && (
        <Modal onClose={() => setEditModal(null)}>
          ...
          <div className="space-y-3">
            ...
            <input
              value={editForm.temp_mail || ""}
              onChange={(e) =>
                setEditForm({
                  ...editForm,
                  temp_mail: e.target.value,
                })
              }
            ...
            <div className="flex justify-end gap-3 pt-4">
              <button onClick={() => setEditModal(null)} className="px-4 py-2">
                Cancel
              </button>
...
const Row = ({ label, value, isPassword }) => {
  ...
  return (
    <div className="flex justify-between items-center text-sm mb-2">
      <div>
        <b>{label}:</b> {isPassword && !show ? "••••••" : value}
      </div>
      ...
      <div className="flex gap-2 items-center">
        ...
        <button onClick={copyText} className="bg-gray-200 px-2 rounded">
          Copy
        </button>
      </div>
    </div>
